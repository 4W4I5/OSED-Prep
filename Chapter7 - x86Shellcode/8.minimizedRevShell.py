...
15jan_1055a: added iterative labelling to better track exec flow
moving onto 7.5 Reverse Shell,

dont forget that besides EBX all registers are volatile i.e will be overwritten

15jan_1102a: for writing a reverse shell, we need 
- a socket to connect to 
- a cmd process to run the commands on

for socket, ws2_32.dll is needed with functions
- WSAStartup
- WSASocketA
- WSAConnect
for cmd process, kernel32.dll is needed with functions
- CreateProcessA
- TerminateProcess
- LoadLibraryA (to load ws2_32.dll)

acc to thebook
1. resolve symbols for CreateProcessA, store addr for later use
2. resolve symbols for ws2_32.dll, using the previous resolve for kernel32


15jan_107p: gotWSAStartup to load into EAX
need to setup args for it to init the ws2_32 lib


15jan_331p: attempting revShell but encountered a funny problem

the nullbyte reverser triggers on push instructions for IP and PORT
port is a small number, IP is fine but port is a problem
currently its fine, but it does tend to add more instructions for no reason
...
27jan_1249a: Got CreateProcessA to spawn cmd.exe, can interact with the shell just fine as well
working on excerises such as pushing STARTUPINFOA to the stack + minimizing the shellcode
